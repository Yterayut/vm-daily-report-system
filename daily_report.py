...
class EnhancedVMReportOrchestrator:
    ...
    def generate_pdf_report(
        ...
        try:
            ...
            # Allow caller-provided service data; fallback to API fetch inside generator.
    ...
    def send_comprehensive_alerts(self, vm_data: list, summary: Dict[str, Any], vm_pdf_path: Optional[Path] = None, service_pdf_path: Optional[Path] = None, 
        ...
        try:
            ...
            # Legacy path stays available by feature flag only.
            use_legacy_email_fix = os.getenv('ENABLE_LEGACY_EMAIL_FIX', 'false').lower() == 'true'

            email_success = False
            if use_legacy_email_fix:
                try:
                    from comprehensive_email_fix import ComprehensiveEmailSender
                    comprehensive_sender = ComprehensiveEmailSender()
                    email_success = comprehensive_sender.send_vm_report(
                        summary, vm_pdf_path, service_pdf_path, service_health_data
                    )
                    if email_success:
                        self.logger.info("✅ Legacy email fix delivery successful")
                except Exception as e:
                    self.logger.warning("⚠️ Legacy email fix unavailable/failed: {}".format(e))

            if not email_success:
                email_success = self._send_professional_email(
                    config, summary, html_content, [vm_pdf_path, service_pdf_path]
                )

            # ALWAYS try to send LINE notification regardless of email status
...
"""
    ...
    def _send_professional_email(
        ...
        try:
            ...
            for attachment in (attachments or []):
                ...
                attached_count += 1
                self.logger.info("✅ PDF attached: {} ({} KB)".format(
            ...
            if email_dry_run:
                self.logger.info(
                    "🧪 EMAIL_DRY_RUN enabled: skip SMTP send (to: {}, attachments: {})".format(
                        len(config['to_emails']), attached_count
                    )
                )
                return True
            ...
            self.logger.info(
                "✅ Email sent successfully to {} recipients ({} attachments)".format(
                    len(config['to_emails']), attached_count
                )
            )
            return True
    ...
    def run_complete_workflow(self) -> bool:
        ...
        try:
            ...
            # Step 1.5: Collect service health data for complete mode
            try:
                service_health_data = get_service_health_data()
                service_alerts = get_service_alerts()
            except Exception as e:
                self.logger.warning("⚠️ Service health collection failed: {}".format(e))
            ...
            alert_success = self.send_comprehensive_alerts(
                vm_data, summary, vm_pdf_path, service_pdf_path, service_health_data, service_alerts
            )
...
def main():
    ...
    # Original comprehensive mode (gated until smoke-test approval)
    non_simple_enabled = os.getenv('ENABLE_NON_SIMPLE_MODE', 'false').lower() == 'true'
    if not non_simple_enabled and not any(flag in sys.argv for flag in ['--simple', '--help', '-h']):
        print("❌ Non-simple mode is temporarily disabled.")
        print("Set ENABLE_NON_SIMPLE_MODE=true after smoke-test validation.")
        return 2
