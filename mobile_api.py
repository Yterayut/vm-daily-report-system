...
try:
    from service_health_adapter import (
        ...
        get_service_alerts,
        get_mobile_services_health_payload,
    )
    SERVICE_HEALTH_AVAILABLE = True
    print("✅ Service health adapter loaded successfully")
except ImportError as e:
    print(f"⚠️ Service health adapter not available: {e}")
    SERVICE_HEALTH_AVAILABLE = False
    get_service_health_data = None
    get_service_alerts = None
    get_mobile_services_health_payload = None
...
try:
    ...
    get_service_health_data_legacy = None
    get_service_alerts_legacy = None


def _get_service_health_data_with_flag():
    use_new_source = os.getenv('ENABLE_NEW_SERVICE_SOURCE', 'false').lower() == 'true'
    if use_new_source and get_service_health_data:
        return get_service_health_data()
    if get_service_health_data_legacy:
        return get_service_health_data_legacy()
    raise RuntimeError("No service health provider available")


def _get_service_alerts_with_flag():
    use_new_source = os.getenv('ENABLE_NEW_SERVICE_SOURCE', 'false').lower() == 'true'
    if use_new_source and get_service_alerts:
        return get_service_alerts()
    if get_service_alerts_legacy:
...
def get_carbon_services_sync():
    ...
    try:
        # Single source of truth for service health (flag-gated).
        use_new_source = os.getenv('ENABLE_NEW_SERVICE_SOURCE', 'false').lower() == 'true'
        if use_new_source and SERVICE_HEALTH_AVAILABLE and get_mobile_services_health_payload:
            return get_mobile_services_health_payload()
        # Legacy behavior
        try:
...
def api_dashboard_enhanced():
    ...
    if SERVICE_HEALTH_AVAILABLE:
        try:
            service_data = _get_service_health_data_with_flag()
        except Exception as e:
            service_data = {
                'services': {},
                'summary': {
    ...
    vm_health_score = vm_data.get('uptime_percentage', 0)
